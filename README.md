<p align="center">
  <img src="resources/tubiao_ui.jpg" width="128" alt="郊狼雷霆图标">
</p>

<h1 align="center">郊狼雷霆</h1>

<p align="center">
  将《战争雷霆》实时遥测与 DG-LAB 郊狼 3.0 联动的 Windows 桌面程序
</p>

<p align="center">
<img src="https://img.shields.io/badge/version-v1.1-4f8fe8" alt="版本 v1.1">
  <img src="https://img.shields.io/badge/platform-Windows%2010%20%2F%2011-2d7dd2" alt="Windows 10 / 11">
  <img src="https://img.shields.io/badge/Python-3.11%2B-3776ab" alt="Python 3.11+">
  <img src="https://img.shields.io/badge/DG--LAB%20App-V3%20%2F%20V4-ef6f91" alt="DG-LAB App V3 / V4">
</p>

> [!WARNING]
> 本项目会控制电刺激设备，使用不当可能造成人身伤害。首次使用前必须完整阅读[《注意事项》](resources/注意事项.txt)，所有通道都应从 `0` 强度开始逐步测试。未成年人、孕妇、心脏疾病患者及植入医疗设备者严禁使用。

## 项目简介

郊狼雷霆（WT-DGLAB）读取《战争雷霆》本机 `8111` 端口提供的实时数据，将空战过载、陆战速度和游戏事件映射为郊狼 A/B 通道的强度与波形。

程序支持 DG-LAB 3.x App 的本机 WebSocket 连接，也支持 DG-LAB 4.x App 的 Socket Relay 连接。用户可以在界面中切换 V3 / V4，选择后会自动保存、重建连接并刷新二维码。

本项目是一个 Vibe Coding 实践作品，代码、协议判断和安全行为仍应以实际测试结果为准。

当前版本为 `v1.1`。协议、界面和自动化回归已经验证；DG-LAB 4.x App 与实际郊狼设备的完整实机联调仍在进行中，请谨慎测试。

## 功能

### 空战模式

- 读取法向过载 `ny`，根据用户设置的 G 值上下限线性映射强度。
- A/B 通道可分别设置最大强度和波形。
- 可为击杀、被击落或坠毁单独设置强度、持续时间与波形。
- 实时仪表盘和透明悬浮窗显示当前 G 值及双通道输出。

### 陆战模式

- 根据载具速度映射电击强度，速度上下限可配置。
- 击杀、被摧毁和维修事件可以使用独立强度与波形。
- 在陆战中进入飞机时自动使用 CAS 过载配置。
- 陆战模式提供“启用 CAS 触发”开关，可同时控制上飞机后的过载和击杀/坠毁事件输出。
- CAS 事件沿用陆战模式的玩家昵称、击杀和坠毁参数，事件显示保留飞机语义。
- CAS、常规陆战和陆战事件配置彼此独立。

### 自定义波形

- 程序启动时自动创建 EXE 同级 `waveforms` 文件夹。
- 将 DG-LAB 官方明文 `.pulse` 文件放入该文件夹，在“参数设置 → 波形设置”中刷新并选择；界面显示完整文件名。
- 仅保留内置“恒定”波形。常规场景可为 A/B 独立设置 5..300 秒随机范围，事件场景可在每次触发时独立随机抽取一次。
- 无有效自定义文件时随机功能自动退化为恒定；解析失败文件会汇总提示并跳过。

### 通用能力

- DG-LAB V3 / V4 App 双协议切换。
- A/B 双通道独立强度，范围 `0..200`。
- 恒定输出、自定义 `.pulse` 波形和按范围随机策略。
- 浅色 PySide6 三栏界面，默认面向 1080p，支持拖动分栏。
- 透明置顶悬浮窗，13–52px 滑块无级调节大小（主窗口与悬浮窗右键均可调），兼顾 1080p 与 2K 屏幕。
- 参数保存、恢复默认和启动自动加载。
- HUD 击杀、被击杀和坠毁事件自动适配英语、俄语、法语、德语、简体中文、繁体中文和日文。
- 点击右上角关闭按钮后缩小到系统托盘，后台联动继续运行；托盘右键选择“退出程序”才会真正退出。
- 重复启动 EXE 时不会创建第二套程序实例，而是恢复并激活已运行的主窗口。
- 关闭到托盘和托盘退出时自动保存当前界面设置，配置文件保存在 EXE 同目录，首次启动会自动生成。
- 程序、任务栏、任务管理器、托盘和界面品牌图统一使用 `tubiao1.jpg` 转换后的图标资源。
- 顶部状态栏“关于”对话框展示应用版本、作者（某不科学的猫酱）、B 站主页与 GitHub 仓库链接，点击链接在系统浏览器打开。
- 数值框与下拉框带滚轮保护，滚动页面时不会误改参数。
- 离开有效对局后清空遥测、取消事件并将 A/B 通道归零。
- 单文件 Windows EXE，用户无需安装 Python 或项目依赖。

## 系统要求

| 项目 | 要求 |
|---|---|
| 操作系统 | 64 位 Windows 10 / 11 |
| 游戏 | Windows 版《战争雷霆》，运行时可访问 `127.0.0.1:8111` |
| 设备 | DG-LAB 郊狼 3.0、电极线与贴片 |
| 手机 | 安装 DG-LAB 3.x 或 4.x App |
| V3 网络 | 手机与电脑位于可互通的同一局域网 |
| V4 网络 | 电脑和手机能够连接所配置的 WebSocket Relay |

## 快速开始

### 使用发布版 EXE

1. 从 GitHub Releases 下载 `WT-DGLAB v1.1.zip` 并完整解压。
2. 启动《战争雷霆》，进入机库或对局。
3. 运行目录中的 `WT-DGLAB.exe`，阅读并确认注意事项。
4. 在右侧“连接郊狼”区域选择 `V3 App` 或 `V4 App`。
5. 使用手机 App 扫描程序显示的二维码。
6. 先将 A/B 最大强度设为 `0`，确认连接、模式和波形正确后再逐步增加。

关闭主窗口右上角的关闭按钮不会停止程序，只会隐藏到 Windows 系统托盘。双击托盘图标可恢复窗口；需要结束程序时，请右键托盘图标并选择“退出程序”。

发布版 ZIP 已包含 Python、PySide6、Pillow、WebSocket 等运行依赖，不需要额外安装环境。请完整解压 ZIP 后运行目录中的 `WT-DGLAB.exe`，不要单独复制 EXE；`_internal` 和 `resources` 目录必须与 EXE 保持相对位置。

### V3 App 连接

1. 在程序右侧选择 `V3 App`，默认本机服务端端口为 `8765`。
2. 确保手机和电脑连接同一局域网，且未开启 AP 隔离或访客网络隔离。
3. 首次运行时，允许 `WT-DGLAB.exe` 通过 Windows 防火墙的“专用网络”。
4. 在 DG-LAB 3.x App 的 Socket / 扫码控制入口扫描二维码。

V3 只需要局域网入站连接，不需要在路由器上配置公网端口映射，也不建议将 `8765` 暴露到公网。若修改了程序中的 V3 端口，防火墙规则也要允许相应 TCP 端口。

### V4 App 连接

1. 在程序右侧选择 `V4 App`，程序会立即保存选择并连接 Relay。
2. 默认 Relay 为 `wss://trex.dungeon-lab.cn/v4`，也可以填写自行部署的 `ws://` 或 `wss://` 地址。
3. 等待二维码刷新后，在 DG-LAB 4.x App 中选择 Socket 控制并扫码。
4. App 接入后，程序会选择首个可用的郊狼设备槽位并先将 A/B 通道归零。

V4 由程序主动连接 Relay，通常不需要开放电脑的 `8765` 入站端口。官方 Relay 不可达时，可以部署官方 `dglab-websocket-server` 并在界面中填写自建地址。

## 参数说明

| 区域 | 参数 | 作用 |
|---|---|---|
| 空战 | G 值下限 / 上限 | 决定过载映射的起点和满强度点 |
| 陆战 | 速度下限 / 上限 | 决定速度映射的起点和满强度点 |
| CAS | G 值与通道设置 | 陆战中进入飞机时使用 |
| A/B 通道 | 最大强度 | 独立限制两个通道，范围 `0..200` |
| 波形 | 恒定 / 预设 / 随机 | 设置常规映射和事件输出的波形 |
| 事件 | 强度 / 持续时间 | 设置击杀、被击落、被摧毁和维修反馈 |
| 玩家昵称 | 游戏内昵称 | 用于区分与玩家相关的 HUD 事件，请保持完全一致；支持的游戏语言会自动识别 |
| 刷新间隔 | `50..1000 ms` | 控制读取 8111 数据的频率，默认 `200 ms` |
| 悬浮窗 | 开关与大小（13–52px 滑块） | 显示当前遥测、事件和 A/B 输出 |

映射采用线性插值，并将最终强度限制在通道最大值内：

```text
value <= min  -> 0
value >= max  -> channel_max
其他          -> (value - min) / (max - min) * channel_max
```

## 安全机制

- 默认配置的 A/B 最大强度和事件强度均为 `0`。
- 未确认处于有效对局时，不使用 `8111` 端口可能残留的速度或过载值。
- 退出对局、游戏断开、切换协议、设备断开或程序关闭时停止相关输出并归零。
- 新 V4 App 或设备刚接入时先清空通道任务并归零。
- 所有输出强度最终限制在 `0..200`。

这些机制不能替代使用者的安全判断。请勿将电极贴在头部、颈部、胸部或破损皮肤上，也不要在驾驶、操作机械或其他危险环境中使用。

## 常见问题

### 查看与导出运行日志

- 点击顶部“注意事项”左侧的 **运行日志**，查看本次启动以来的记录。日志窗口可与主界面同时操作，支持选中复制；向上翻阅时暂停跟随，回到底部恢复。
- 点击 **导出本次日志**，将截至点击时刻的完整日志保存到程序旁的 `logs/` 文件夹；源码运行时位于项目根目录。点击 **打开日志文件夹** 可直接查看文件。
- `run_启动时间_进程号.log` 是本次自动日志。**正常退出且清理成功时删除**；关闭到托盘或关闭日志窗口不会删除，也不会停止记录。
- 崩溃、强制结束、未处理异常或退出清理失败时保留自动日志；下次启动不覆盖、不清理旧日志，可使用记事本打开。强制结束或断电时，最后少量记录可能来不及写入。
- `export_启动时间_导出时间_唯一编号.log` 是手动导出的文件，**不会自动删除**。多次导出分别保存，历史异常日志和导出文件由用户自行删除。
- 窗口最多显示最近 10,000 行，正常情况下导出仍包含本次完整日志。目录不可写、磁盘已满等故障会显示“日志保存失败”，此时导出只能保存仍可用的最近内存记录，并明确标记为不完整。
- 日志记录启停、连接变化、关键设置、事件和异常堆栈；强度变化按一秒汇总，不逐帧输出游戏数据。

### 战争雷霆一直显示未连接

- 确认游戏正在运行，并尝试在浏览器打开 `http://127.0.0.1:8111/state`。
- `8111` 是游戏提供的本机接口，通常不需要手动开放防火墙端口。
- 返回 `{"valid": false}` 通常表示当前不在有效对局，程序会保持安全归零。
- **开着加速器或 Clash 时**：浏览器对 `127.0.0.1` 有内置绕行，因此浏览器能打开不代表程序也正常。程序新版本已强制直连本机地址，若仍失败请更新到最新包，并检查代理软件是否拦截回环请求。
- 程序日志会记录 8111 不可达的具体原因（连接被拒 / 超时 / HTTP 状态码），反馈问题时请附上日志。

**浏览器能打开首页，但程序仍显示未连接时：**

| 可能原因 | 新版日志提供的检查证据 |
| --- | --- |
| 运行了旧包，或请求受到系统代理影响 | 启动记录诊断标识 `8111-diag-1`、源码/EXE 运行方式、程序位置及代理存在性；新版全部遥测请求强制直连。代理存在并不等于故障已确认。 |
| 首页正常，但 `/state` 返回错误、重定向、HTML 或异常 JSON | 分接口记录 HTTP 状态、Content-Type、JSON 顶层类型、错误和恢复；拒绝跟随重定向。 |
| 0.5 秒超时过短或服务间歇繁忙 | 记录失败类型、耗时和阈值；连续三次失败后，在独立后台任务中用连接/读取各 2 秒的超时比较首页与 `/state`。 |
| 8111 没有监听、被其他进程占用，或安全软件按进程拦截 | 记录连接错误和 Windows 监听地址、PID、进程名；拦截只能作为排查方向，无法仅凭网络错误确认。 |
| 地图/载具接口异常，或后续解析、界面更新出错 | 记录 `/map_info.json`、`/indicators`、`/hudmsg` 的异常及恢复，并保留读取/界面异常堆栈。 |

机库、主菜单或 `valid=false` **不应单独导致新版显示未连接**：`/state` 正常时连接灯仍可为绿色，但未经确认处于有效对局时保持零输出。首页和仪表数据也可能残留，建议在同一台电脑、同一时刻直接检查 `/state`。

诊断探测最多每分钟一次，不阻塞正常轮询，不改变连接灯或设备输出；普通接口日志按原因变化去重。复现后，在退出程序前点击 **运行日志 → 导出本次日志** 并附上导出文件。旧 EXE 不会自动获得这些检查，需要包含新代码的发布包；仅凭 `v1.0` 标题不能确认是否包含修复。

### V3 扫码后无法连接

- 确认手机和电脑在同一网段，而不是访客 Wi-Fi。
- 允许 `WT-DGLAB.exe` 通过 Windows 专用网络防火墙。
- 确认程序显示的 IP 是手机能够访问的局域网地址。
- 检查 V3 端口是否被其他程序占用。
- 不要为此配置公网端口转发。

### V4 一直停在连接 Relay

- 检查电脑能否访问所填的 `ws://` 或 `wss://` 地址。
- 官方 Relay 不可用时尝试自建 Relay。
- 切回 V3 再切换到 V4 会自动重建连接并刷新二维码，无需点击“保存设置”。

### 刚退出对局后数值被清空

这是预期的安全行为。战争雷霆在退出对局后可能保留 `/indicators` 的旧速度，程序以 `/map_info.json` 的有效状态作为输出门槛，避免继续使用残留数据。

### EXE 为什么有约 270 MB

发布版完整收集了 PySide6 / Qt 运行库和插件，以提高免安装运行的兼容性。单文件 EXE 已包含程序所需依赖，体积较大不代表安装后还需要额外环境。

## 从源码运行

### 准备环境

```powershell
git clone https://github.com/Unscientificcat/WT-DGLAB.git
cd WT-DGLAB

python -m venv .venv
.\.venv\Scripts\Activate.ps1
python -m pip install --upgrade pip
python -m pip install -r requirements.txt
python main.py
```

项目要求 Python 3.11 或更高版本。当前开发与打包环境使用 Python 3.14。

### 运行测试

```powershell
python -m pip install pytest
python -m pytest -q
```

当前完整自动化测试结果为 `51 passed`。实际手机、游戏和电刺激设备仍需要人工联调。

### 打包

```powershell
python build.py
```

打包脚本使用 PyInstaller `--onedir --windowed`，在 `dist/` 生成 `WT-DGLAB v1.1/` 目录及同名 ZIP。目录包含 `WT-DGLAB.exe`、`_internal/`、`resources/`、安全默认模板、README 和 LICENSE。用户配置只保存在 EXE 同目录的 `config.json`；新版本不会自动迁移旧目录配置。

## 架构

```text
战争雷霆 :8111
       |
       v
  GameReader ---> EventDetector
       |               |
       +-------> MappingEngine
                        |
                  MainWindow / Overlay
                        |
          +-------------+-------------+
          |                           |
          v                           v
 CoyoteController              CoyoteV4Controller
 V3 本机 WS 服务端              V4 Relay 客户端
          |                           |
          v                           v
 DG-LAB 3.x App                 Relay -> DG-LAB 4.x App
          |                           |
          +------------ BLE ----------+
                         |
                    郊狼 3.0
```

主要模块：

| 文件 | 职责 |
|---|---|
| `main.py` | 生命周期、线程队列、UI 与控制器协调 |
| `src/game_reader.py` | 读取并解析战争雷霆 `8111` 数据 |
| `src/event_detector.py` | HUD 击杀 / 死亡事件与维修边沿检测 |
| `src/mapping_engine.py` | G 值 / 速度到 A/B 强度的线性映射 |
| `src/coyote_controller.py` | DG-LAB V3 本机 WebSocket 服务端 |
| `src/coyote_v4_controller.py` | DG-LAB V4 Relay 控制客户端 |
| `src/gui/` | PySide6 主窗口、样式、悬浮窗和注意事项 |
| `src/config_manager.py` | `config.json` 配置加载、保存与默认值 |

更详细的技术资料见 [docs/tech-spec.md](docs/tech-spec.md)、[docs/ui-redesign.md](docs/ui-redesign.md) 和 [docs/v4-app-adaptation.md](docs/v4-app-adaptation.md)。

## 更新日志

### v1.1

- 新增运行日志窗口，支持实时查看、复制和导出本次日志。
- 正常退出删除本次自动日志；异常日志和手动导出文件保留在程序旁的 `logs/`。
- 所有战争雷霆遥测请求强制直连，修复系统代理可能导致读取失败的问题。
- 增加接口状态、耗时、JSON 格式、后台连通性和端口监听进程诊断，便于排查“浏览器可打开但程序未连接”。
- 改善重复日志去重、强度汇总与退出清理。

### v1.0

首个正式版。自 beta 系列以来的主要更新：

- 新增浅色磨砂玻璃 UI：默认壁纸背景、玻璃卡片、可调卡片不透明度，支持自定义背景图。
- 仪表盘业务模式切换（空战/陆战）移至实时状态卡，切换立即生效并持久化。
- 实时输出波形显示：通道卡与悬浮窗显示当前输出波形名，点击通道卡可打开单通道输出电压曲线窗口。
- 悬浮窗显示内容可调：模式、实时过载、实时速度、事件提示、A/B 强度与波形名共 8 项开关。
- 悬浮窗大小改为无级滑块调节，旧配置自动迁移。
- 新增“关于”对话框，版本号统一由 `src/version.py` 管理。
- 完整自动测试 `175 passed`。

### v1 beta_6

- 仪表盘 A/B 通道卡与悬浮窗实时显示当前输出波形名（恒定或波形文件名），随机换波、事件换波、CAS 切换时同步跟随。
- 点击左侧仪表盘 A/B 通道强度卡片，打开该通道独立的输出电压曲线窗口：滚动显示最近约 6 秒的实际下发电压（0-200 刻度与强度条一致），含波形名、强度与状态读出。
- 新增悬浮窗显示内容设置：可选显示模式、实时过载、实时速度、事件提示、A/B 通道强度与波形名（共 8 项，默认全开）；入口为主窗口悬浮窗卡片按钮和悬浮窗右键菜单，改动即时生效并保存。
- 完整自动测试 `155 passed`。

### v1 beta_2

- HUD 击杀、被击杀和坠毁事件新增英语、俄语、法语、德语、简体中文、繁体中文和日文自动识别。
- 区分主动与被动事件格式，并增加 Unicode、零宽字符、空白和大小写标准化。
- 七种语言的击杀、死亡和坠毁回归测试已通过，完整自动测试为 `98 passed`。
- 窗口标题、Windows 应用标识和发布产物统一更新为 `WT-DGLAB v1 beta_2`。

### v1 beta_1

本次版本整理了 2026-08-15 晚间完成的稳定性和陆战 CAS 更新，内容包括：

- 增加系统托盘生命周期：关闭主窗口只隐藏到托盘，托盘菜单可恢复窗口或真正退出。
- 增加配置持久化：设置自动保存到 EXE 同目录的 `config.json`，首次运行自动生成。
- 增加 Windows 单实例和本地 IPC；重复启动时恢复并激活已有窗口，不再创建第二套轮询、设备连接和托盘。
- 修复维修期间触发击杀后维修输出不能自动恢复的问题。
- 增加陆战“启用 CAS 触发”开关，可独立控制上飞机后的过载、击杀和坠毁输出。
- 修复陆战 CAS 使用错误事件配置导致击杀和坠毁事件不触发的问题。
- 完成图标、窗口标题、任务栏和任务管理器品牌标识统一，版本标识为 `郊狼雷霆 v1 beta_1`。
- 完成根目录与发布目录回归测试，当时共 `51` 项通过，并生成 `WT-DGLAB v1 beta_1.exe`。

### v1 beta

相较 `v0.1`，本版本主要更新如下。

#### 全新界面

- GUI 从 tkinter 全面迁移到 PySide6。
- 重做为浅色三栏控制界面，覆盖主窗口、实时仪表盘、参数设置、二维码、悬浮窗和注意事项。
- 默认适配 1080p，并优化 2K 屏幕下的悬浮窗大档尺寸。
- 数值框和下拉框不再响应悬停滚轮，避免滚动页面时误改参数。
- 发布包改为携带完整 Qt 运行库，免安装兼容性提高，但 EXE 由 `v0.1` 的约 26 MB 增至当前约 270 MB。

#### DG-LAB 4.x App 支持

- 在保留 V3 本机 WebSocket 模式的基础上新增 V4 Socket Relay 控制器。
- 支持官方 Relay 和用户自建 Relay。
- V3 / V4 切换后自动保存、重建控制器并刷新二维码，不再需要额外点击保存。
- 网络连接移出 Qt 主线程，Relay 不可达时不会阻塞界面。
- 修复切换协议时 V3 服务残留异步任务的问题。

#### 遥测和事件检测重写

- 新增独立 `EventDetector`，统一处理 HUD 游标、击杀、被击落、被摧毁和维修事件。
- 持续消费战争雷霆跨局递增的 `lastDmg`，菜单阶段只推进游标，新对局首批事件可以及时触发。
- 修复旧 HUD 历史记录在保存设置、断开或重进对局后重复触发的问题。
- 修复事件刚入队时被空遥测误取消，以及击杀倒计时停在 `5.0s` 的问题。

#### 对局退出安全归零

- 不再仅依赖 `/state` 或速度是否为零判断对局状态。
- 使用 `/map_info.json` 有效性作为输出门槛，拦截菜单中残留的速度和载具数据。
- 离开有效对局时清空仪表与悬浮窗、取消未结束事件，并立即将 A/B 通道归零。

#### 悬浮窗与稳定性

- 修复悬浮窗开关和大 / 中 / 小尺寸按钮不立即生效的问题。
- 悬浮窗改为真正透明背景，并保留置顶与拖动。
- 增大“大”档字号，提高 2K 屏幕可读性，同时保留适合 1080p 的中 / 小档。
- 修复反复打开和关闭注意事项对话框可能导致程序闪退的问题。
- 修复战争雷霆位于前台时，启动主窗口和注意事项被游戏窗口覆盖的问题。

#### 设置、品牌与工程质量

- 将刷新间隔移动到左侧仪表盘底部，V3 / V4 连接参数集中在右侧。
- 修复部分波形与随机间隔配置重启后未正确加载的问题。
- 版本号更新为 `v1 beta`，更新窗口、任务栏、任务管理器和 EXE 图标。
- 增加系统托盘生命周期：关闭按钮隐藏窗口，托盘菜单负责恢复和真正退出。
- 配置改为保存在 EXE 同目录，缺失时自动创建，并在关闭到托盘或退出时保存当前控件值。
- 使用 `tubiao1.jpg` 生成新的多尺寸 Windows 图标和界面品牌图片。
- 增加单实例互斥和本地 IPC，重复启动时只恢复已有窗口，避免重复轮询和设备连接。
- 修复维修期间触发击杀后，击杀倒计时结束却无法恢复维修输出的问题。
- 增加陆战 CAS 总开关；关闭后上飞机不再输出过载、击杀或坠毁事件。
- 修复陆战 CAS 使用空战事件配置导致击杀和坠毁事件无法按陆战设置触发的问题。
- 建立 V3 / V4、遥测安全、事件检测、界面、托盘、单实例和品牌资源回归测试，当前共 `51` 项。

### v0.1

- 首个可运行版本。
- 支持战争雷霆空战过载与陆战速度映射。
- 支持 DG-LAB 3.x App、本机 WebSocket、A/B 通道、基础波形和事件设置。
- 使用 tkinter 界面，仅提供 V3 连接路径。

## 已知限制

- 当前主要面向 Windows 与郊狼 3.0，未验证其他硬件型号。
- DG-LAB V4 Relay 握手、二维码和协议帧已验证，但 4.x App 与实际设备的完整实机联调仍待完成。
- 战争雷霆接口字段可能随游戏更新或载具类型变化，事件识别也依赖游戏 HUD 文本。
- 项目仍处于 beta 阶段，不建议在无人看护或高强度场景中使用。

## 发布前注意

- `config.json` 可能包含玩家昵称和个人强度设置，公开仓库前应确认其中没有隐私信息。
- `wt-dglab-trace.log` 属于诊断日志，不应作为正式发布内容。
- 建议通过 GitHub Releases 分发 `WT-DGLAB v1.1.zip`，不要要求普通用户从源码构建。

## 相关资料

- [战争雷霆 localhost:8111 文档](https://github.com/lucasvmx/WarThunder-localhost-documentation)
- [DG-LAB 郊狼 3.0 蓝牙协议](https://github.com/dungeonlab-open/dglab-bluetooth-protocol/blob/main/coyote/v3/README.md)
- [DG-LAB Python Kit](https://github.com/dungeonlab-open/dglab-kit-python)
- [DG-LAB WebSocket Relay](https://github.com/dungeonlab-open/dglab-websocket-server)

## 许可证与声明

本项目采用 [MIT License](LICENSE) 开源。除非另有说明，项目代码均按 MIT License 授权；使用、修改和再发布时请保留许可证及版权声明。

本项目是非官方第三方工具，与 Gaijin Entertainment、War Thunder、DG-LAB 或地牢实验室不存在隶属或授权关系。相关名称和商标归各自权利人所有。
